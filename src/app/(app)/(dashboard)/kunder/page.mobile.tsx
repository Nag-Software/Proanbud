...
import { CustomerDetailsDrawer } from '@/components/kunder/CustomerDetailsDrawer.mobile';
import { NewCustomerDrawer } from '@/components/kunder';
...
export default function KunderPage() {
            ...
            if (customersSnapshot.exists()) {
              ...
              firebaseCustomers = Object.entries(customerData)
                ...
                .map(([key, data]) => ({
                  ...
                  // Guard against missing or invalid dates so the listener doesn't throw and hide all customers
                  sistAktivitet: (() => {
                    if (!data || data.sistAktivitet === undefined || data.sistAktivitet === null) return '';
                    const d = new Date(data.sistAktivitet);
  ...
  return (
    <div className="w-full min-h-full px-0">
        ...
        <div className="flex flex-rows justify-between px-2 pt-2">
            <PageHeader title="Alle Kunder" />
            <button   
            onClick={handleNewCustomer}
            disabled={(isLimited && !hasTrialAccess) || !checkCustomerLimit().canProceed || limitsLoading}
            className={`flex items-center cursor-pointer gap-3 px-3 py-2 h-10 rounded-lg transition-colors ${
